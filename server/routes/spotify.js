...
async function getAccessTokenForUserId(userId) {
  ...
  if (!accessToken || Date.now() >= (expiresAt - 60000)) {
    ...
    const tokenResp = await axios.post(SPOTIFY_ACCOUNTS + '/api/token', params.toString(), {
...
router.get('/connect', (req, res) => {
  ...
  const authUrl = SPOTIFY_ACCOUNTS + '/authorize?client_id=' + spotifyClientId + '&response_type=code&redirect_uri=' + encodeURIComponent(redirectUri) + '&scope=' + encodeURIComponent(scopes) + '&state=' + state;
...
router.get('/callback', async (req, res) => {
  ...
  try {
    const tokenResponse = await axios.post(SPOTIFY_ACCOUNTS + '/api/token', 
      new URLSearchParams({ 
    ...
    const { data: existingAccount } = await supabaseAdmin.from('spotify_accounts').select('id').eq('spotify_user_id', spotifyUserId).single();
    ...
    res.redirect(`${frontendUrl}/register-complete?error=spotify_auth_failed`);
  }
});
...
router.get('/current-playing', async (req, res, next) => {
  try {
    ...
    const spotifyResp = await axios.get(SPOTIFY_API + '/me/player/currently-playing', { 
      headers: { Authorization: `Bearer ${accessToken}` } 
    });

    if (spotifyResp.status === 204 || !spotifyResp.data?.item) return res.json({ playing: false });

    const item = spotifyResp.data.item;
    ...
    res.json({
      ...
      release_date: item.album?.release_date || null, // We will use this for the Era filter!
      progress_ms: spotifyResp.data.progress_ms,
...
// ... top-tracks, top-artists, and stats routes remain unchanged from before ...
router.get('/top-tracks', async (req, res, next) => {
  try {
    ...
    const resp = await axios.get(SPOTIFY_API + '/me/top/tracks?time_range=' + timeRange + '&limit=50', { 
      headers: { Authorization: `Bearer ${accessToken}` } 
    });

    const tracks = (resp.data.items || []).map((t, i) => ({
      rank: i + 1,
      name: t.name,
...
router.get('/top-artists', async (req, res, next) => {
  try {
    ...
    const spotifyResp = await axios.get(SPOTIFY_API + '/me/top/artists', { 
      headers: { Authorization: `Bearer ${accessToken}` } 
    });
...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    const [topTracksResp, topArtistsResp, recentResp] = await Promise.all([
      axios.get(SPOTIFY_API + '/me/top/tracks', { headers: { Authorization: `Bearer ${accessToken}` } }),
      axios.get(SPOTIFY_API + '/me/top/artists', { headers: { Authorization: `Bearer ${accessToken}` } }),
      axios.get(SPOTIFY_API + '/me/player/recently-played', { headers: { Authorization: `Bearer ${accessToken}` } })
    ]);

    res.json({
      topTracks: (topTracksResp.data.items || []).map(item => ({
        id: item.id,
        name: item.name,
        artists: item.artists.map(a => a.name).join(', '),
        image: item.album?.images?.[0]?.url || null
      })),
      topArtists: (topArtistsResp.data.items || []).map(a => ({
        id: a.id,
        name: a.name,
        genres: a.genres || [],
        image: a.images?.[0]?.url || null
